				...
				<div class="col-md-8 col-md-offset-2 text-center">
					<div class="display-t">
						<div class="display-tc animate-box" data-animate-effect="fadeIn">
							<h1>Ending the project...</h1>
							<?php
								$project_id = intval($_GET["project_id"]);
								$db = pg_connect("host=localhost port=5432 dbname=CrowdFunding user=postgres password=370305");

								$result = pg_query($db, "UPDATE project_initiated_by SET status = 'EXPIRED' WHERE project_id = $project_id");
								if(!$result){
									echo '<meta http-equiv="refresh" content="1.5; URL=endMyProjectFailure.php" />';
								}else{
									echo '<meta http-equiv="refresh" content="1.5; URL=endMyProjectSuccess.php" />';
								}
							?>
						</div>
					</div>
