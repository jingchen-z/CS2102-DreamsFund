			...
			<div class="row animate-box">
				<div class="col-md-8 col-md-offset-2 text-center fh5co-heading">
					<form action="#" method="POST" autocomplete="off">
								...
								<input type="text" name="old_password" id="old_password" class="form-control" placeholder="Enter your old password">
								</div>
					...
					<?php
						...
						if(isset($_POST['Save'])) {
							$result1 = pg_query($db, "SELECT * FROM users WHERE email = '$userEmail'");
							$row = pg_fetch_assoc($result1);
							if(!empty($row)){
								if($row[transaction_password] == $_POST[old_password]){
									$result2 = pg_query($db, "UPDATE users SET transaction_password = '$_POST[new_password]' WHERE email = '$_SESSION[user_email]'");
									if (!$result2) {
							            echo '<meta http-equiv="refresh" content="0; URL = editUserInfoFailure.php" />';
							        } else {
							            echo '<meta http-equiv="refresh" content="0; URL = editUserInfoSuccess.php" />';
							        }
								}else{
									echo '<meta http-equiv="refresh" content="0; URL = editUserInfoFailure.php" />';
								}	
